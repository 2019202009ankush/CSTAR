...
class Block {
  ...
  static createBlock(lastBlock, data, wallet) {
    ...
    var tree = merkle('sha1').sync(data);
    let merkel_hash=tree.root();
  ...
  static blockHash(block) {
    const { timestamp, lastHash, data , merkel_hash } = block;
    return Block.hash(timestamp, lastHash, data,merkel_hash);
  }
  ...
  static verifyBlock(block) {
    return ChainUtil.verifySignature(
      block.proposer,
      block.signature,
      Block.hash(block.timestamp, block.lastHash, block.data,block.merkel_hash)
    );
  }
