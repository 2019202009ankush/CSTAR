  ...
  messageHandler(socket) {
    ...
    socket.on("message", message => {
      ...
      switch (data.type) {
		...
		let d = "\nstartTime  "+Date.now()+"  "+process.env.P2P_PORT;
		  
		// Write data in 'Output.txt' . 
		fs.appendFile('Output.txt', d, (err) => { 
		      
		    // In case of a error throw err. 
		    if (err) throw err; 
		}) 
                console.log("+++++++++++start time+++++++++++",Date.now(),process.env.P2P_PORT);
              }
            } else {
		...
		// Data which will write in a file. 
		let d = " endTime  "+Date.now()+"  "+process.env.P2P_PORT;
		...
		fs.appendFile('Output.txt', d, (err) => { 
		    ...
		    if (err) throw err; 
              ...
              console.log("+++++++++++End time++++++++++++",Date.now(),process.env.P2P_PORT);
            }
